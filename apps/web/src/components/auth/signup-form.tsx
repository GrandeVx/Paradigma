...
export function SignUpForm({
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send OTP for verification (this will also create the user if not exists)
  ...
  const handleSocialAuth = async (provider: "google" | "apple") => {
    try {
      ...
      if (result.error) {
        toast({
          title: "Error",
          description: result.error.message || `Failed to sign up with ${provider}`,
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <form onSubmit={step === "form" ? handleFormSubmit : handleOtpSubmit}>
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-6">
            ...
            <Button type="submit" className="w-full" disabled={isLoading}>
              {isLoading ? "Loading..." : step === "form" ? "Create Account" : "Verify & Sign In"}
            </Button>
          </div>
